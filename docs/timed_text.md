...
```bash
raw2bmx -t op1a -o output.mxf -y 10:00:00:00 --avci video.h264 --wave audio.wav --timed-text manifest.txt
```
...
```bash
raw2bmx -t imf -o {Type}_{fp_uuid}.mxf -f 25 --dur 100 --timed-text manifest.txt
```
